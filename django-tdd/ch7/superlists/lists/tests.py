...
class ListViewTest(TestCase):
    def test_displays_only_items_for_that_list(self):
        ...
        item1_text = "itemey 1"
        item2_text = "itemey 2"
        Item.objects.create(text=item1_text, list=correct_list)
        Item.objects.create(text=item2_text, list=correct_list)

        other_list = List.objects.create()
        other_item1_text = "other list item 1"
        other_item2_text = "other list item 2"
        Item.objects.create(text=other_item1_text, list=other_list)
        Item.objects.create(text=other_item2_text, list=other_list)

        response = self.client.get("/lists/%d/" % correct_list.id)

        self.assertContains(response, item1_text)
        self.assertContains(response, item2_text)
        self.assertNotContains(response, other_item1_text)
        self.assertNotContains(response, other_item2_text)
